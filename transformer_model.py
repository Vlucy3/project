...
train_loader = DataLoader(SalesDataset(X_train_seq, y_train_seq),
                          batch_size=BATCH, shuffle=True,  num_workers=0)
val_loader   = DataLoader(SalesDataset(X_val_seq,   y_val_seq),
...
class TransformerForecaster(nn.Module):
    """
    Encoder-only Transformer for univariate-target tabular time series.
    Input : (batch, seq_len, n_features)
    Output: (batch, 1)  — prediction for the next step
    """
...
for epoch in range(1, EPOCHS + 1):
    ...
    if epoch % 5 == 0 or epoch == 1:
        print(f"  Epoch {epoch:>3} | train MSE {train_loss:.5f} | "
              f"val MSE {val_loss:.5f} | lr {optimizer.param_groups[0]['lr']:.2e}")
...
ax.set_title("Transformer — Training & Validation Loss")
ax.set_xlabel("Epoch")
ax.set_ylabel("MSE (log-space)")
ax.legend()
plt.tight_layout()
plt.savefig(DATA_DIR / "transformer_training_curve.png", dpi=150)
plt.show()
print("Training curve saved to transformer_training_curve.png")

# ── 11. SAVE MODEL ────────────────────────────────────────────────────────────
torch.save(
    {"model_state": best_weights, "scaler": scaler, "features": FEATURES,
     "d_model": 64, "nhead": 4, "num_layers": 2, "dim_feedforward": 256},
    DATA_DIR / "transformer_model.pt",
)
...
out_path = DATA_DIR / "transformer_submission.csv"
